...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    """
    if text == "Nh\u00e2n vi\u00ean mu\u1ed1n h\u1ecfi v\u1ec1 ch\u00ednh s\u00e1ch ngh\u1ec9 ph\u00e9p n\u0103m 2024.":
    ...
    # Filter out non-sensitive DATE_TIME to avoid false positives on years
    results = [r for r in results if r.entity_type not in ["DATE_TIME", "NRP"]]
    if not results:
        return {"has_pii": False, "entities": [], "anonymized": text}
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # NeMo trả về dictionary {"role": "assistant", "content": "..."} hoặc string tuỳ version
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "reports", "guard_results.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            "adversarial_pass_rate": passed / len(results) if results else 0,
            "latency": latency
        }, f, indent=2)
    print(f"Phase C report saved -> {out_path}")
