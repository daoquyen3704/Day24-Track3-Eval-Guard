...
if __name__ == "__main__":
    ...
    # --- Bias report ---
    report = bias_report([result])
    print(f"\nBias report: {report}")

    out_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "reports", "judge_results.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"kappa": kappa, "bias_report": report}, f, indent=2, ensure_ascii=False)
    print(f"Phase B report saved -> {out_path}")
