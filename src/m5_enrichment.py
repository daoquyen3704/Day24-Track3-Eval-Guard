                    ...
                    {"role": "system", "content": """Phân tích đoạn văn và trả về JSON:
{
  ...
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
    ...
    questions = [f"{s.rstrip('.')}?" for s in sentences_for_q[:3]]
...
def enrich_chunks(
    ...
    if os.path.exists(cache_path):
        try:
            ...
            with open(cache_path, encoding="utf-8") as f:
                cache = _json.load(f)
            print(f"  [I]  Loaded {len(cache)} cached enriched chunks.")
        except Exception as e:
    ...
    for i, chunk in enumerate(chunks):
        ...
        # Check cache
        if use_combined and text in cache:
            result = cache[text]
    ...
    # Write cache
    if cache_updated and use_combined:
        try:
            import json as _json
            os.makedirs(os.path.dirname(cache_path), exist_ok=True)
            with open(cache_path, "w", encoding="utf-8") as f:
                _json.dump(cache, f, ensure_ascii=False, indent=2)
            print(f"  [OK] Saved {len(cache)} enriched chunks to cache.")
        except Exception as e:
